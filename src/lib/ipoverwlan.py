# Implementation of a dummy wireless LAN module handling multiple
...
def setup(dispatch):
    ...
    for faceindex in range(len(settings.WLAN_FACE_IDS)):
...
class Worker(Thread):
    ...
    def run(self):
        ...
        facereg.face_id = settings.WLAN_FACE_IDS[faceindex]
        facereg.face_module_name = settings.WLAN_MODULE_NAME
        ...
        encap.from_direction_module_name = settings.WLAN_MODULE_NAME
        encap.from_face_id = settings.WLAN_FACE_IDS[faceindex]
...
class Handler:
    ...
    def handle_msg(encap):
        ...
        if type(encap.packet_contents) is Interest:
            ...
            # log
            logmsg = settings.WLAN_MODULE_NAME + ':Interest received to be sent '
        ...
        # build ContentObject message
            ...
            # log
            logmsg = settings.WLAN_MODULE_NAME + ':ContentObj received to be sent '
            common.log_activity(logmsg)

        # unknown message
        else:
            # log
            logmsg = settings.WLAN_MODULE_NAME + ':Unknown message received to sent '
            common.log_activity(logmsg)
            return
        ...
        found = False
        faceindex = None
        for i in range(len(settings.WLAN_FACE_IDS)):

            # if face is valid (exists) save it and exit loop
            if settings.WLAN_FACE_IDS[i] == encap.to_face_id:
                faceindex = i
                found = True
                break
        ...
        if found:
            ...
            ipaddress, port = settings.WLAN_IP_CONNECTIONS[i].split(':')
            
            # open socket, send the created and close socket
            pass

            # log
            logmsg = settings.WLAN_MODULE_NAME + ':Unknown face specified '
            common.log_activity(logmsg)
